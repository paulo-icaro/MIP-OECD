...
dicio_isic = read_excel(path ='C:/Users/paulo.costa.SDE/Downloads/Tese/Dicionario_ISIC.xlsx')
...
dicio_isic_ncm = read_excel(path = 'C:/Users/paulo.costa.SDE/Downloads/Tese/Dicionario_NCM_ISIC.xlsx')
dicio_isic_ncm_agro = dicio_isic_ncm %>% filter(CO_ISIC_CLASSE %in% dicio_isic_agro$CO_ISIC_CLASSE)

# Dicionario NCM
dicio_NCM = read_excel(path = 'C:/Users/paulo.costa.SDE/Downloads/Tese/Dicionario_NCM.xlsx')
...
# Exportacoes
expo = read.csv(file = 'C:/Users/paulo.costa.SDE/Downloads/Tese/Exportacoes_Completas_(1997-2024).csv', sep = ';')   # Base - Exportacoes
agro_expo = expo %>%  filter(CO_NCM %in% dicio_isic_ncm_agro$CO_NCM)                                # Base - Exportacoes - Agropecuaria
total_expo = expo[c(1,11)] %>% group_by(CO_ANO) %>% summarize(Total_Expo = sum(VL_FOB))             # Exportacoes p/ Ano - Total
...
# Importacoes
impo = read.csv(file = 'C:/Users/paulo.costa.SDE/Downloads/Tese/Importacoes_Completas_(1997-2024).csv', sep = ';')   # Base - Importacoes
...
agro_expo_ranking = agro_expo %>% group_by(CO_ANO, CO_NCM) %>% summarize(VL_EXPO = sum(VL_FOB)) %>% slice_max(order_by = VL_EXPO, n = 5)
agro_expo_ranking = left_join(x = agro_expo_ranking, y = dicio_NCM[c(1,12)], by = 'CO_NCM')
